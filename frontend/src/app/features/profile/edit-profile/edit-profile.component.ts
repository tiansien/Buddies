...
export class EditProfileComponent implements OnInit {
  ...
  loading = true;
  uploading = false;
  defaultProfilePicture = 'assets/images/defaultProfilePicture.jpg';
  ...
  getUserAndProfileById(userId: string): void {
    this.profileService.getUserAndProfileById(userId).subscribe({
      next: (data) => {
        this.user = data[0];
        this.profile = data[1] || {};
        // Set profile picture preview to user's photo or default
  ...
  saveProfile(): void {
    ...
    // If no new file is selected and we're using the default picture
    if (
      !this.selectedFile &&
      this.profilePicturePreview === this.defaultProfilePicture
    ) {
      // Convert default image to File object
      ...
      // Normal update with selected file or keeping existing photo
